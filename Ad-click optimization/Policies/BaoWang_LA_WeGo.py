...
class Policy_BaoWang_LA_WeGo(Policy):
    ...
    def bid(self, attr):
        ...
        est_KG = np.zeros(len(mu))

        future_profit = [[0]*len(self.prob_win) for _ in range(len(self.bid_space))]
        for x in range(len(mu)):
            for y in range(len(self.prob_win)):
                future_profit[x][y] = (self.gamma[attr] * self.beta[attr] - self.bid_space[x])*self.lamb[attr] * self.alpha[attr] * self.p_x(self.bid_space[x], y, attr)

        for j in range(len(mu)):
            KG = np.zeros(self.samples)
            for i in range(self.samples):
                auct = np.random.poisson(self.lamb[attr])
                p = 0
                for k in range(len(self.theta0)):
                    p = p + self.prob_win[k] * self.p_x(self.bid_space[j], k, attr)
                imp = np.random.binomial(auct, p)

                # projecting out to the future, simulating prob_win
                prob_win_future = [1 / len(self.theta0)] * len(self.theta0)
                for k in range(len(self.theta0)):
                    prob_win_future[k] = np.power(self.p_x(self.bid_space[j], k, attr), imp) * np.power(1-self.p_x(self.bid_space[j], k, attr), auct - imp) * self.prob_win[k]
                prob_win_future = np.divide(prob_win_future, np.sum(prob_win_future))

                mu_future = [0]*len(self.bid_space)
                for x in range(len(mu)):
                    mu_future[x] = np.dot(prob_win_future,future_profit[x])

                KG[i] = np.max(mu_future) - np.max(mu)
            est_KG[j] = (1/self.samples) * np.sum(KG)
        big_N = 167  # simulator runs 167 iterations
        little_N = self.iter

        index = np.argmax(np.add(mu, np.multiply((big_N - little_N), est_KG)))
    ...
    def learn(self, info):
        ...
        for result in info:
            ...
            if result['num_click'] > 0:
                ...
                self.beta[attr] = 0.9*self.beta[attr] + (0.1)*(self.beta[attr] * self.cumClicks[attr] + self.C_t[attr]) / (self.cumClicks[attr] + self.K_t[attr])
