...
class Policy_BreyerJohnson_PS_5(Policy):
    ...
    def initialize_belief(self):
        ...
        for i in range(14):
            for j in range(10):
                ...
                self.var[i, j] = np.sqrt(prob * (1-prob) / (self.alpha[i,j] + self.beta[i,j]))
    ...
    def bid(self, attr):
        ...
        opt = np.argmax(prob_array + self.var[a_ix, :] * theta)
        decision = int(round(opt))
        if decision < 0:
            decision = 0
        if decision > 9:
            decision = 9
