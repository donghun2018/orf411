...
class MedicalDecisionDiabetesModel():
    ...
    def __init__(self, state_names, x_names, s_0, exog_info_fn=None, transition_fn=None, objective_fn=None, seed=20180529):
        ...
        self.obj = 0.0
        self.obj_sum = 0.0
        self.sigma_w = 0.0 # changed later
    ...
    def exog_info_fn(self, decision):
        mu = np.random.normal(getattr(self.state, decision)[0], \
                              1 / math.sqrt(getattr(self.state, decision)[1]))
        W = np.random.normal(mu, self.sigma_w)
        beta_W = Beta(self.sigma_w)
        return {"reduction": W, "beta_W": beta_W, "mu": mu}
    ...
    def objective_fn(self, decision, exog_info):
        ...
        return mu # changed this from W for the sake of plotting mu in the value.
