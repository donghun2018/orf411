...
class MDDMPolicy():
    ...
    def UCB(self, state_dict, time_elapsed, theta):
        ...
        for key in state_dict:
            # set the number of times we have tested the policy to be 1 for each drug. (specifically for this policy)
            # write dictionary of stats
            # +5 to account for assuming each drug has been tested once
            # Also we set the number of times we have tested the policy to be 1 for each drug. (specifically for this policy)
            aux_stats[key] = [state_dict[key][0], math.sqrt(math.log(time_elapsed + 1)/(state_dict[key][2] + 1))]
            stats[key] = aux_stats[key][0] + theta*aux_stats[key][1]
        optimal_decision = max(stats, key=stats.get)
        # print(aux_stats[optimal_decision])
        return optimal_decision

    def IE(self, state_dict, time_elapsed, theta):
        # This method implements the Interval Estimation policy
        stats = {}
        aux_stats = {}
        for key in state_dict:
            # write dictionary of stats
            aux_stats[key] = [state_dict[key][0], state_dict[key][1]]
            stats[key] = (state_dict[key][0] + \
                 theta*state_dict[key][1])
        optimal_decision = max(stats, key=stats.get)
        return optimal_decision
    
    def PureExploitation(self, state_dict, time_elapsed, theta):
    ...
    def PureExploration(self, state_dict, time_elapsed, theta):
        # This method implements the Pure Exploration policy (random drug every time)
        stats = {}
        for key in state_dict:
            stats[key] = state_dict[key][0]
        optimal_decision = random.choice(list(stats))
        return optimal_decision
